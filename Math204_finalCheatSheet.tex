...
\subsubsection{Kruskal-Wallis test (assume CRD)}
...
\subsubsection{Friedman test (assume RBD)}
...
\begin{tabular}{cccccc}
 ...
 \hline Treat & $K-1$ & $\mathrm{SST}$ & $\mathrm{MST}$ & $\frac{\mathrm{MST}}{\mathrm{MSE}}$ & $P\left(\mathcal{F}_{K-1, n-K}>F_{o b s}\right)$ \\
 Error & $n-K$ & SSE & $\mathrm{MSE}$ & & \\
\hline Total & $n-1$ &$SS_{tot}$ &$=SSE$&$+SST$ 
\end{tabular}

\subsubsection{Comparing means}
...
Bonferroni: experiment-wise error $\alpha_E \implies$ comparison-wise error $\alpha_E/C$ where C is the \# of comparisons.
