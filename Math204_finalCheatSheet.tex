...
\item Std error (slope) : $\hat{\sigma}_{\hat{\beta}_1}$
...
Note: $SST$ and $SS_{Tot}$ stay the same whether the blocks are included or not.
\begin{tabular}{lcccc}
Source & df & SS & MS & $F$ \\
...
\subsubsection{Conditions for all ANOVA tests}
1) Random and independent samples of experimental units are
associated with each treatment.

2) The probability distribution of responses for each $JK$ factor-level combination (treatment) is approximately Normal.

3) The $JK$ factor-level combination (treatment) probability distributions have equal variances.
